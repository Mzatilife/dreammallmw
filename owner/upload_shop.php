...
$shop = new ShopsContr;
// Checking if the shop ID is set ---------------------------->
...
if (isset($_POST['upload'])) {
    ...
    $shop_id = TestInput($_SESSION['shop_id']);
    $nego = TestInput($_POST['nego']);
    $description = TestInput($_POST['description']);

    $st = $shop->uploadShopProduct($shop_id, $name, $quantity, $nego, $price, $description);
    if ($st) {
        $imageday = time();
        ...
        foreach ($_FILES['images']['name'] as $key => $val) {
            ...
            $final = $shop->uploadImages($prod_id, $new_image_name);
        }
        if ($final) {
            $msg = "Product added!";
        } else {
...
<head>
    ...
    <title>Upload Shop Product</title>
                    ...
                    <div class="col-md-12">
                        <div class="card shadow mb-4">
                            <div class="card-header">
                                <strong class="card-title">Upload Shop Products | Services</strong>
                            </div>
                            <div class="card-body">
                                <?php
                                if (isset($msg)) { ?>
                                ...
                                <form class="needs-validation" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data" novalidate>
                                    <div class="form-row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" name="name" class="form-control required" id="validationCustom3" placeholder="Shoes" required>
